...
fn valid(n: u64, preamble: &[u64], size: usize) -> bool {
    assert_eq!(preamble.len(), size);
    ...
    for x in preamble.iter() {
        ...
        candidates.insert(x);
        //println!("n:{} x:{} v:{} {:?}", n, x, v, candidates);
        if v != *x && candidates.contains(&v) {
            return true;
        }
    }
    return false;
...
fn main() {
    ...
    for line in stdin.lock().lines() {
        ...
        let number = sline.parse::<u64>().unwrap();
        numbers.push(number);
    }
    let n = 25;
    let mut weak = 0u64;
