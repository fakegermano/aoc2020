...
fn main() {
    ...
    for (idx, inp) in input.iter().enumerate() {
        ...
        v.push_front(idx as i32);
        turns.insert(*inp, v.to_owned());
        _last = *inp;
        //println!("{} {}", idx, _last);
    }
    ...
    for i in input.len()..2020 {
        if turns.get(&last).unwrap().len() == 1 {
            ...
            if turns.contains_key(&last) {
                turns.get_mut(&last).unwrap().push_front(i as i32);
            } else {
                let mut v = LinkedList::new();
                v.push_front(i as i32);
                turns.insert(last, v.to_owned());
            }
        } else {
            let mut ages = turns.get(&last).unwrap().iter();
            let age = ages.next().unwrap() - ages.next().unwrap();
            last = age;
            if turns.contains_key(&last) {
                turns.get_mut(&last).unwrap().push_front(i as i32);
            } else {
